...
class Agent:
    ...
    def max(self, current_board, current_color, depth, alpha, beta):
        if self.game.check_terminal(current_board, current_color):
            return self.game.evaluate(current_board, current_color, -1000)
        ...
        for move in possible_moves:
            temporary_move, value = self.min(current_board.next_board(current_color, move),  self.game.opponent(current_color), depth + 1, alpha, beta)
    ...
    def min(self, current_board, current_color, depth, alpha, beta):
        ...
        if depth == self.max_depth:
            return self.game.evaluate(current_board, current_color)
        ...
        for move in possible_moves:
            temporary_move, value = self.max(current_board.next_board(current_color, move), self.game.opponent(current_color), depth + 1, alpha, beta)
